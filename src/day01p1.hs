...
-- | Process the instructions in order and returns the number of times that an
-- instruction goes to 0
process :: State -> [Instruction] -> Int
process st [] = zeroVisits st
process st ((Instruction L value):is)
  = let newPosition = (position st) - (mod value 100)
        adjNewPosition = if (newPosition < 0) then (100 + newPosition) else newPosition
        newZeroVisits = if (adjNewPosition == 0) then (zeroVisits st) + 1 else (zeroVisits st)
